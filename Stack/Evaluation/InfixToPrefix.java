...
public class InfixToPrefix {
    ...
    public InfixToPrefix(String infix) {
        ...
        sb.reverse();   //reversing the string with the help of the stringBuilder concept      

        for(int i = 0; i < sb.length(); i++){
        if(sb.charAt(i) == '(')
            sb.setCharAt(i, ')');
        else if(sb.charAt(i) == ')')
            sb.setCharAt(i, '(');   //changing the position of the brackets
        }
        
        this.infix=sb.toString();
    ...
    public void conversion(){
        ...
        for(int i=0;i<size;i++){
         ...
         while(!sa.isEmpty()){
            ...
            System.out.println("The prefix of the infix expression is="+finalResult.reverse());
    }
}
